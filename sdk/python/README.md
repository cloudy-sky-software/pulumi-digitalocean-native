...
This boilerplate creates a working Pulumi-owned provider named `digitalocean-native`.
...
1. Set the following options:
   - Owner: pulumi
   - Repository name: pulumi-digitalocean-native-native (replace "digitalocean-native" with the name of your provider)
   - Description: Pulumi provider for digitalocean-native
   - Repository type: Public
1. Clone the generated repository.

From the templated repository:

1. Search-replace `digitalocean-native` with the name of your desired provider.
...
```bash
$ cd examples/simple
$ yarn link @pulumi/digitalocean-native
$ yarn install
...
1. A `provider/` folder containing the building and implementation logic
   1. `cmd/`
      1. `pulumi-gen-digitalocean-native/` - generates language SDKs from the schema
      2. `pulumi-resource-digitalocean-native/` - holds the package schema, injects the package version, and starts the gRPC server
   2. `pkg`
...
3. `sdk` - holds the generated code libraries created by `pulumi-gen-digitalocean-native/main.go`
4. `examples` a folder of Pulumi programs to try locally and/or use in CI.
5. A `Makefile` and this `README`.

### Writing the schema

The [JSON schema file](https://www.pulumi.com/docs/guides/pulumi-packages/schema) is used by `pulumi-gen-digitalocean-native` to create language-specific SDKs.
...
This repository provides the [digitalocean-native example schema](./provider/cmd/pulumi-resource-digitalocean-native/schema.json) to get you started.  
...
type digitalocean-nativeProvider struct {
...
func makeProvider(host *provider.HostClient, name, version string, pulumiSchema []byte) (pulumirpc.ResourceProviderServer, error) {
   ...
   return &digitalocean-nativeProvider{
   host:    host,
   name:    name,
   version: version,
   schema:  pulumiSchema,
